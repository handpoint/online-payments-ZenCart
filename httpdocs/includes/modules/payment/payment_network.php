...
require(__DIR__ . '/payment_network/StageOrder.php');
class payment_network {
    ...
    function __construct() {
        global $order, $db;
        $this->code = 'payment_network';
        $this->version = MODULE_PAYMENT_PAYMENT_NETWORK_ADMIN_TITLE;
        $this->description = MODULE_PAYMENT_PAYMENT_NETWORK_ADMIN_DESCRIPTION;
        $this->secret = MODULE_PAYMENT_PAYMENT_NETWORK_MERCHANT_SECRET ?: 'Circle4Take40Idea';
//        $this->secret = 'Threeds2Test60System';
        ...
        if (IS_ADMIN_FLAG === true && !$this->enabled && MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE == 'Direct'){
            $warning = MODULE_PAYMENT_PAYMENT_NETWORK_ADMIN_WARNING;
            $this->title .= "<span class=\"alert\" title=\"$warning\">" . substr($warning, 0, 32) . "...</span>";
        }

        // Set display order
        $this->sort_order = MODULE_PAYMENT_PAYMENT_NETWORK_SORT_ORDER;

        if ((int)MODULE_PAYMENT_PAYMENT_NETWORK_ORDER_STATUS_ID > 0) {
            $this->order_status = MODULE_PAYMENT_PAYMENT_NETWORK_ORDER_STATUS_ID;
        }
    ...
    function update_status() {
        ...
        if (($this->enabled == true) && ((int)MODULE_PAYMENT_PAYMENT_NETWORK_ZONE > 0)) {
            $check_flag = false;
            $check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PAYMENT_NETWORK_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
            while (!$check->EOF) {
    ...
    function selection() {
        if (MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE == 'Direct' || MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE == 'Direct V2') {
    ...
    function process_button() {
        switch (MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE) {
            case 'Hosted':
            case 'Modal':
    ...
    public function create_direct_request() {
        global $order;
        return array(
            "merchantID" => MODULE_PAYMENT_PAYMENT_NETWORK_MERCHANT_ID,
            "action" => "SALE",
            "type" => 1,
            "transactionUnique" => uniqid(),
            "currencyCode" => MODULE_PAYMENT_PAYMENT_NETWORK_CURRENCY_ID,
            "amount" => intval($order->info['total'] * 100),
            "orderRef" => strftime("%d/%m/%y %H:%M") . " - " . self::generate_random_string(),
            "cardNumber" => $_POST['payment_network_card_number'],
            "cardExpiryMonth" => $_POST['payment_network_card_expires_month'],
            "cardExpiryYear" => $_POST['payment_network_card_expires_year'],
            "cardCVV" => $_POST['payment_network_card_cvv'],
            "customerName" => $_POST['payment_network_card_holder'],
            ...
            "countryCode" => MODULE_PAYMENT_PAYMENT_NETWORK_COUNTRY_ID,
    ...
    public function create_hosted_request() {
        ...
        $db->Execute("DELETE FROM payment_network_temp_carts WHERE payment_network_cdate <= NOW() - INTERVAL 2 HOUR");
        // Upload session that contains their cart to table called `payment_network_temp_carts`
        $db->Execute("INSERT INTO payment_network_temp_carts (`payment_network_orderRef`, `payment_network_session`, `payment_network_orderID`) VALUES (\"$ref\", \"$session\", NULL)");
        return array(
            "merchantID"        => MODULE_PAYMENT_PAYMENT_NETWORK_MERCHANT_ID,
            "amount"            => intval($order->info['total'] * 100),
            "countryCode"       => MODULE_PAYMENT_PAYMENT_NETWORK_COUNTRY_ID,
            ...
            "callbackURL"       => ($this->is_https() ? HTTPS_SERVER . DIR_WS_HTTPS_CATALOG : HTTP_SERVER . DIR_WS_CATALOG) . 'payment_network_callback.php',
    ...
    function before_process() {
        global $db;
        $_POST = (isset($_SESSION['PAYMENT_NETWORK_CALLBACK']) ? $_SESSION['PAYMENT_NETWORK_CALLBACK'] : $_POST);
        // Implement behaviour for the two modules
        if (MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE == 'Hosted' || MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE == 'Modal') {
            if (self::has_keys($_POST, self::get_response_template())) {
                // Respond to the users redirect request the same way as a callback
                $this->res = $_POST;
                // Check if we came back via the redirect (PAYMENT_NETWORK_CALLBACK WILL NOT BE SET!)
                if (!defined('PAYMENT_NETWORK_CALLBACK')) {
                    // Get the status of the callback
                    $results = $db->Execute("SELECT * FROM payment_network_temp_carts WHERE payment_network_orderRef = \"" . $this->res['orderRef'] . "\"");
                    if ($results->fields['payment_network_orderID'] !== null) {
                        // Make sure to prevent any duplicates...
                        $id = intval($results->fields['payment_network_orderID']);
                        define('PAYMENT_NETWORK_CALLBACK_ID', $id);
        ...
        if (MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE == 'Direct') {

            if (!isset($_REQUEST['MD'], $_REQUEST['PaRes'])) {
                $req = $this->create_direct_request();
                $this->res = $this->make_request(MODULE_PAYMENT_PAYMENT_NETWORK_DIRECT_URL, $req);
        ...
        if (MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE == 'Direct V2') {
...
HTML;
    ...
    function process_all() {
        ...
        if (isset($this->res['signature'])){
            ...
            if($sig != $this->create_signature($this->res, $this->secret)){
                $error = MODULE_PAYMENT_PAYMENT_NETWORK_VERIFY_ERROR;
                error_log($error);
            } elseif (intval($this->res['responseCode']) != 0) {
                $error = sprintf(MODULE_PAYMENT_PAYMENT_NETWORK_RESPONSE_ERROR, htmlentities($this->res['responseMessage']));
            }
        } else {
            $error = MODULE_PAYMENT_PAYMENT_NETWORK_VERIFY_ERROR;
            error_log($error);
    ...
    function after_order_create($zf_order_id) {
        ...
        $db->Execute("UPDATE " . TABLE_ORDERS . " SET " .
            "payment_network_xref = \"" . $this->res['xref'] . "\", " .
            "payment_network_transactionUnique = \"" . $this->res['transactionUnique'] . "\", " .
            "payment_network_amount_received = " . floatval($this->res['amountReceived']) / 100 . ", " .
            "payment_network_authorisationCode = \"" . $this->res['authorisationCode'] . "\", " .
            "payment_network_responseMessage = \"" . $this->res['responseMessage'] . "\", " .
            "payment_network_lastAction = \"" . $this->res['action'] . "\", " .
            "orders_status = " . MODULE_PAYMENT_PAYMENT_NETWORK_ORDER_STATUS_ID . " " .
            "WHERE orders_id = $zf_order_id"
        );
        // Always update carts to prevent duplicates
        $db->Execute("UPDATE payment_network_temp_carts
            SET payment_network_orderID = $zf_order_id
            WHERE payment_network_orderRef = \"{$this->res['orderRef']}\""
        );
...
HTML;
    ...
    function module_title() {
        ...
        if ((defined('IS_ADMIN_FLAG') && IS_ADMIN_FLAG === true) || (!isset($_GET['main_page']) || $_GET['main_page'] == '')) {
            // In Admin mode
            return MODULE_PAYMENT_PAYMENT_NETWORK_ADMIN_TITLE;
        } else {
            // In Catalog mode
            return MODULE_PAYMENT_PAYMENT_NETWORK_CATALOG_TEXT_TITLE;
        }
    }
    ...
    function valid_setup() {
        $isEnabled = MODULE_PAYMENT_PAYMENT_NETWORK_STATUS == 'True';
        // Make sure that the PAYMENT_NETWORK module is enable and that we're running HTTPS on a direct capture type
        return ($isEnabled && in_array(MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE, array('Direct', 'Direct V2'), true) && $this->is_https() || $isEnabled && in_array(MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE, array('Hosted', 'Modal'), true));
    }
    /*
    ...
    function form_url() {
        switch (MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE) {
            case 'Hosted':
                return MODULE_PAYMENT_PAYMENT_NETWORK_FORM_URL;
            case 'Modal':
                return MODULE_PAYMENT_PAYMENT_NETWORK_MODAL_URL;
            case 'Direct':
            case 'Direct V2':
    ...
    function draw_direct_form() {
        ...
        $selection = array(
            ...
            'fields' => array(
                array(
                    'title' => MODULE_PAYMENT_PAYMENT_NETWORK_CARD_HOLDER,
                    'field' => zen_draw_input_field('payment_network_card_holder', $order->billing['firstname'] . ' ' . $order->billing['lastname'], 'id="payment_network-cc-owner"' . $onFocus . ' autocomplete="off"'),
                    'tag'   => 'payment_network-cc-owner'
                ),
                array(
                    'title' => MODULE_PAYMENT_PAYMENT_NETWORK_CARD_NUMBER,
                    'field' => zen_draw_input_field('payment_network_card_number', $ccnum, 'id="payment_network-cc-number"' . $onFocus . ' autocomplete="off"'),
                    'tag'   => $this->code . '-cc-number'
                ),
                array(
                    'title' => MODULE_PAYMENT_PAYMENT_NETWORK_CARD_EXPIRE,
                    'field' => zen_draw_pull_down_menu('payment_network_card_expires_month', $expires_month, strftime('%m'), 'id="payment_network-cc-expires-month"' . $onFocus) . '&nbsp;' . zen_draw_pull_down_menu('payment_network_card_expires_year', $expires_year, '', 'id="payment_network-cc-expires-year"' . $onFocus),
                    'tag'   => 'payment_network-card-expires-month'
                ),
                array(
                    'title' => MODULE_PAYMENT_PAYMENT_NETWORK_CARD_CVV,
                    'field' => zen_draw_input_field('payment_network_card_cvv', '', 'size="4" maxlength="4"' . ' id="payment_network-cc-cvv"' . $onFocus . ' autocomplete="off"') . ' ' . '<a href="javascript:popupWindow(\'' . zen_href_link(FILENAME_POPUP_CVV_HELP) . '\')">' . MODULE_PAYMENT_PAYMENT_NETWORK_CARD_CVV_HELP . '</a>',
                    'tag'   => 'payment_network-card-cvv'
                )
            )
        );
    ...
    function draw_direct_form_button($includeDeviceData = true) {
        $process_button_string = zen_draw_hidden_field('payment_network_card_holder', $_POST['payment_network_card_holder']) .
            zen_draw_hidden_field('payment_network_card_expires', $this->card['expiry_month'] . substr($this->card['expiry_year'], -2)) .
            zen_draw_hidden_field('payment_network_card_expires_month', $this->card['expiry_month']) .
            zen_draw_hidden_field('payment_network_card_expires_year', substr($this->card['expiry_year'], -2)) .
            zen_draw_hidden_field('payment_network_card_type', $this->card['card_type']) .
            zen_draw_hidden_field('payment_network_card_number', $this->card['card_number']) .
            zen_draw_hidden_field('payment_network_card_cvv', $_POST['payment_network_card_cvv']);
...
SCRIPT;
    ...
    function card_data_check() {
        ...
        $result        = $cc_validation->validate($_POST['payment_network_card_number'], $_POST['payment_network_card_expires_month'], $_POST['payment_network_card_expires_year']);
        $error         = '';
        switch ($result) {
            case -1:
        ...
        if (($result == false) || ($result < 1)) {
            $payment_error_return = 'payment_error=payment_network';
            $error_info2          = '&error=' . urlencode($error) . '&payment_network_card_holder=' . urlencode($_POST['payment_network_card_holder']) . '&payment_network_card_expires_month=' . $_POST['payment_network_card_expires_month'] . '&payment_network_card_expires_year=' . $_POST['payment_network_card_expires_year'];
            $messageStack->add_session('checkout_payment', $error . '<!-- [payment_network] -->', 'error');
            zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
    ...
    function admin_notification($zf_order_id)
    {
        ...
        require(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/payment_network/payment_network_admin_notification.php');
    ...
    function _doRefund($oID, $amount = 'Full', $note = '')
    {
        ...
        if ($transaction_info->RecordCount() < 1) {
            $messageStack->add_session(MODULE_PAYMENT_PAYMENT_NETWORK_TEXT_NO_MATCHING_ORDER_FOUND, 'error');
            $proceedToRefund = false;
        }

        // Check user ticked the confirm box
        if (isset($_POST['refconfirm']) && $_POST['refconfirm'] != 'on') {
            $messageStack->add_session(MODULE_PAYMENT_PAYMENT_NETWORK_TEXT_REFUND_CONFIRM_ERROR, 'error');
            $proceedToRefund = false;
        }
        // Check user gave a valid refund amount
        if (isset($_POST['refamt']) && (float)$_POST['refamt'] == 0) {
            $messageStack->add_session(MODULE_PAYMENT_PAYMENT_NETWORK_TEXT_INVALID_REFUND_AMOUNT, 'error');
            $proceedToRefund = false;
        }
        ...
        $req = array(
            ...
            'xref' => $transaction_info->fields['payment_network_xref'],
            'merchantData' => $this->version

        );

        $res = $this->make_request(MODULE_PAYMENT_PAYMENT_NETWORK_DIRECT_URL,$req);

        if(isset($res['responseCode']) && $res['responseCode'] == 0){

            if($_POST['refamt'] == $order->info['total']){
                $new_order_status = MODULE_PAYMENT_PAYMENT_NETWORK_REFUNDED_STATUS_ID;
            }elseif($order->info['total'] > $_POST['refamt']){
                $new_order_status = MODULE_PAYMENT_PAYMENT_NETWORK_PART_REFUNDED_STATUS_ID;
            }else{
                $new_order_status = MODULE_PAYMENT_PAYMENT_NETWORK_REFUNDED_STATUS_ID;
            }

            $sql_data_array= array(array('fieldName'=>'orders_id', 'value' => $oID, 'type'=>'integer'),
                array('fieldName'=>'orders_status_id', 'value' => MODULE_PAYMENT_PAYMENT_NETWORK_REFUNDED_STATUS_ID, 'type'=>'integer'),
                array('fieldName'=>'date_added', 'value' => 'now()', 'type'=>'noquotestring'),
                array('fieldName'=>'comments', 'value' => MODULE_PAYMENT_PAYMENT_NETWORK_REFUND_DEFAULT_MESSAGE." {$_POST['refamt']}", 'type'=>'string'),
            ...
            $messageStack->add_session(sprintf(MODULE_PAYMENT_PAYMENT_NETWORK_TEXT_REFUND_INITIATED, $res['transactionID'], $oID), 'success');
    ...
    function check()
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PAYMENT_NETWORK_STATUS'");
    ...
    function install()
    {
        ...
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Payment Network Module', 'MODULE_PAYMENT_PAYMENT_NETWORK_STATUS', 'True', 'Do you want to accept Payment Network payments?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Select Integration Method', 'MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE', 'Hosted', 'Do you want to use Direct (SSL Required), Hosted or Modal', '6', '2', 'zen_cfg_select_option(array(\'Hosted\', \'Modal\', \'Direct\', \'Direct V2\'), ', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Merchant ID', 'MODULE_PAYMENT_PAYMENT_NETWORK_MERCHANT_ID', 'TEST', 'Merchant ID set in your mms', '6', '3', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Merchant Secret', 'MODULE_PAYMENT_PAYMENT_NETWORK_MERCHANT_SECRET', 'TEST', 'Merchant signature secret as set in mms', '6', '4', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Payment Name.', 'MODULE_PAYMENT_PAYMENT_NETWORK_CATALOG_TEXT_TITLE', 'Card Payment', 'Name of payment method shown to customer', '6', '5', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Display Order.', 'MODULE_PAYMENT_PAYMENT_NETWORK_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '6', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Currency ID.', 'MODULE_PAYMENT_PAYMENT_NETWORK_CURRENCY_ID', '826', 'ISO currency number', '6', '7', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Country ID.', 'MODULE_PAYMENT_PAYMENT_NETWORK_COUNTRY_ID', '826', 'ISO currency number', '6', '8', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Responsive Hosted Layout', 'MODULE_PAYMENT_PAYMENT_NETWORK_RESPONSIVE_TYPE', 'True', 'Use responsive layout on a hosted form?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");

        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Order Status', 'MODULE_PAYMENT_PAYMENT_NETWORK_ORDER_STATUS_ID', '2', 'Set the status of orders paid with this payment module to this value. <br /><strong>Recommended: Processing[2]</strong>', '6', '25', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Unpaid Order Status', 'MODULE_PAYMENT_PAYMENT_NETWORK_ORDER_PENDING_STATUS_ID', '1', 'Set the status of unpaid orders made with this payment module to this value. <br /><strong>Recommended: Pending[1]</strong>', '6', '25', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Refund Order Status', 'MODULE_PAYMENT_PAYMENT_NETWORK_REFUNDED_STATUS_ID', '1', 'Set the status of refunded orders to this value. <br /><strong>Recommended: Pending[1]</strong>', '6', '25', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
        $db->Execute("REPLACE INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Set Partial Refund Order Status', 'MODULE_PAYMENT_PAYMENT_NETWORK_PART_REFUNDED_STATUS_ID', '2', 'Set the status of partially refunded orders to this value. <br /><strong>Recommended: Processing[2]</strong>', '6', '25', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
        $result = $db->Execute("
            SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_NAME = '" . TABLE_ORDERS . "'
            AND TABLE_SCHEMA = '" . DB_DATABASE . "'
            AND COLUMN_NAME IN ('payment_network_xref', 'payment_network_transactionUnique', 'payment_network_amount_received', 'payment_network_authorisationCode', 'payment_network_responseMessage', 'payment_network_lastAction')
        ");

        if (intval($result->fields['COUNT(*)']) < 1) {
            $db->Execute("ALTER TABLE " . TABLE_ORDERS . "
                ADD COLUMN `payment_network_xref` VARCHAR(128) NULL,
                ADD COLUMN `payment_network_transactionUnique` VARCHAR(128) NULL,
                ADD COLUMN `payment_network_amount_received` FLOAT NOT NULL DEFAULT '0.0',
                ADD COLUMN `payment_network_authorisationCode` VARCHAR(128) NULL,
                ADD COLUMN `payment_network_responseMessage` TEXT NULL,
                ADD COLUMN `payment_network_lastAction` VARCHAR(32) NULL
            ");
        }

        $db->Execute("CREATE TABLE IF NOT EXISTS payment_network_temp_carts (payment_network_orderRef VARCHAR(64) NOT NULL, payment_network_session TEXT NOT NULL, payment_network_cdate TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, payment_network_orderID int NULL)");
    ...
    {
        return array(
            'MODULE_PAYMENT_PAYMENT_NETWORK_STATUS',
            'MODULE_PAYMENT_PAYMENT_NETWORK_CAPTURE_TYPE',
            'MODULE_PAYMENT_PAYMENT_NETWORK_RESPONSIVE_TYPE',
            'MODULE_PAYMENT_PAYMENT_NETWORK_MERCHANT_ID',
            'MODULE_PAYMENT_PAYMENT_NETWORK_MERCHANT_SECRET',
            'MODULE_PAYMENT_PAYMENT_NETWORK_CATALOG_TEXT_TITLE',
            'MODULE_PAYMENT_PAYMENT_NETWORK_CURRENCY_ID',
            'MODULE_PAYMENT_PAYMENT_NETWORK_COUNTRY_ID',
            'MODULE_PAYMENT_PAYMENT_NETWORK_SORT_ORDER',
            'MODULE_PAYMENT_PAYMENT_NETWORK_ORDER_STATUS_ID',
            'MODULE_PAYMENT_PAYMENT_NETWORK_ORDER_PENDING_STATUS_ID',
            'MODULE_PAYMENT_PAYMENT_NETWORK_REFUNDED_STATUS_ID',
            'MODULE_PAYMENT_PAYMENT_NETWORK_PART_REFUNDED_STATUS_ID'
        );
    }
